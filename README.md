...
This repository provides a pipeline for **converting the USLCI database into a Brightway2-compatible format** and for **adding custom/hand-crafted LCA datasets into an existing USLCI Brightway2 project**.

**The main script is `backup_plan.py`** — it reads `uslci.csv`, cleans the data, builds the full Brightway2 database, adds all technosphere and biosphere exchanges, runs matrix diagnostics, and exports LCIA results. Everything else in the repo supports either loading a pre-built project or extending it with new datasets.
...
```
.
├── backup_plan.py                   # *** MAIN PIPELINE SCRIPT *** Build USLCI BW2 DB from CSV
├── helper_functions.py              # Shared utilities used by backup_plan.py (unit conversions, issue logging)
...
```bash
# 1. Clone the repo
git clone https://github.com/modeling-sustainability/bw2uslci.git
cd bw2uslci
...
# 3. Set your Brightway2 data directory
#    Open backup_plan.py (and helper_functions.py) and update this line near the top:
#    os.environ['BRIGHTWAY2_DIR'] = "/Users/tghosh/Desktop/bw2uslci_test/"
#    Change the path to wherever you want Brightway2 to store its project data.

# 4. Run the pipeline
python backup_plan.py
```

After a successful run, results are written to the `output/` directory — see the Output Files section below.
...
> **`BRIGHTWAY2_DIR`:** Both `backup_plan.py` and `helper_functions.py` set this path explicitly near the top of the file (e.g., `"/Users/tghosh/Desktop/bw2uslci_test/"`). Update it to a directory on your own machine before running. You can also set it in your shell before running any script:
>
> ```bash
...
## Workflow A — Restore the Pre-Built USLCI Project (Alternative to running backup_plan.py)
